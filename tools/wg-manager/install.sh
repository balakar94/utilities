...
PREFIX="/usr/local"
APPLY=0; YES=0; SUDO=0; FORCE=0; UNINSTALL=0; RESTORE=0; DRYRUN=0
usage() {
...
while [ $# -gt 0 ]; do
  case "$1" in
    --prefix)
      if [ "$#" -lt 2 ]; then echo "ERROR: --prefix requires a path value." >&2; usage >&2; exit 2; fi
      case "${2:-}" in
        --*) echo "ERROR: --prefix requires a path value, got flag: $2" >&2; exit 2 ;;
        "") echo "ERROR: --prefix requires a non-empty path." >&2; exit 2 ;;
      esac
      PREFIX="$2"; shift 2 ;;
...
case "$PREFIX" in
  *[!A-Za-z0-9._/+:-]*)
    echo "ERROR: --prefix contains unsafe characters: $PREFIX" >&2; exit 2 ;;
esac
...
PKG_MGR="unknown"; TIER="2"
if [ -f /etc/os-release ]; then
  # shellcheck disable=SC1091
  . /etc/os-release
  OSID="${ID:-unknown}"; OSLIKE="${ID_LIKE:-}"
  case " $OSID $OSLIKE " in
    *" debian "*|*" ubuntu "*) PKG_MGR="apt"; TIER="1" ;;
    *" rhel "*|*" fedora "*|*" alma "*|*" rocky "*|*" ol "*|*" centos "*|*" nobara "*) PKG_MGR="dnf"; TIER="1" ;;
    *" arch "*|*" endeavouros "*|*" cachyos "*|*" manjaro "*) PKG_MGR="pacman"; TIER="1" ;;
  esac
fi
...
if [ -z "${SRC_VER:-}" ]; then echo "ERROR: cannot read source version." >&2; exit 1; fi
INST_VER="none"
...
if [ "$UNINSTALL" -eq 1 ]; then
  if [ "$APPLY" -eq 0 ]; then
    ...
    echo "PLAN: removes the binary only; packages, services and state are untouched."; exit 0
  fi
  if [ "$YES" -eq 0 ]; then echo "ERROR: --uninstall --apply requires --yes." >&2; exit 2; fi
  PRIV=""
  if [ "$(id -u)" -ne 0 ]; then
    if [ "$SUDO" -eq 0 ]; then echo "ERROR: need root or --sudo." >&2; exit 2; fi
    sudo -n true || { echo "ERROR: sudo -n failed." >&2; exit 1; }
    PRIV="sudo -n"
  fi
  # Audit fix: A9 - uninstall only removes; use --restore to bring a backup back.
  $PRIV rm -f "$BIN"
  echo "Uninstalled $BIN (packages, services, state untouched)."; exit 0
fi
if [ "$RESTORE" -eq 1 ]; then
  LATEST="$(ls -1 "$BIN".bak.* 2>/dev/null | sort | tail -n 1 || true)"
  if [ "$APPLY" -eq 0 ]; then
    echo "PLAN: restore ${LATEST:-none} to $BIN (dry-run, changes nothing)."; exit 0
  fi
  if [ "$YES" -eq 0 ]; then echo "ERROR: --restore --apply requires --yes." >&2; exit 2; fi
  if [ -z "${LATEST:-}" ] || [ ! -f "$LATEST" ]; then echo "ERROR: no backup found for $BIN." >&2; exit 1; fi
  PRIV=""
  if [ "$(id -u)" -ne 0 ]; then
    if [ "$SUDO" -eq 0 ]; then echo "ERROR: need root or --sudo." >&2; exit 2; fi
    sudo -n true || { echo "ERROR: sudo -n failed." >&2; exit 1; }
  ...
  $PRIV cp -p "$LATEST" "$BIN"; $PRIV chmod 0755 "$BIN"
  echo "Restored $LATEST to $BIN."; exit 0
fi
if [ "$INST_VER" = "$SRC_VER" ] && [ "$FORCE" -eq 0 ]; then echo "OK: $BIN already at $SRC_VER (use --force)."; exit 0; fi
...
if [ "$APPLY" -eq 0 ]; then echo "Dry-run: changes nothing. Re-run with --apply --yes [--sudo]."; exit 0; fi
if [ "$YES" -eq 0 ]; then echo "ERROR: --apply requires --yes." >&2; exit 2; fi
PRIV=""
if [ "$(id -u)" -ne 0 ]; then
  if [ "$SUDO" -eq 0 ]; then echo "ERROR: need root or --sudo." >&2; exit 2; fi
  sudo -n true || { echo "ERROR: sudo -n failed." >&2; exit 1; }
...
HAD_FILE=0; if [ -e "$BIN" ]; then HAD_FILE=1; $PRIV cp -p "$BIN" "$BACKUP"; $PRIV chmod 0600 "$BACKUP"; fi
case "$PKG_MGR" in
  apt) $PRIV apt-get update && $PRIV apt-get install -y $PKGS ;;
  dnf) $PRIV dnf install -y $PKGS ;;
  pacman) $PRIV pacman -S --needed --noconfirm $PKGS ;;
  *) echo "ERROR: unsupported distro for apply." >&2; exit 1 ;;
esac
...
if [ "$VERIFY" != "pass" ]; then
  ...
  if [ "$HAD_FILE" -eq 1 ]; then $PRIV cp -p "$BACKUP" "$BIN"; $PRIV chmod 0755 "$BIN"; else $PRIV rm -f "$BIN"; fi
  exit 1
fi
